...
class UnifiedResourceServer:
    ...
    def fulfill(self, x_payment_header: str) -> tuple[int, dict]:
        ...
        parse X-PAYMENT → TrustGate (hard gate) → SettlementEngine → result.
        Returns (http_status, body). Never raises on untrusted input."""
        try:
            auth = self.adapter.parse_x_payment_header(x_payment_header)
        except ValueError:
            return 400, {"error": "Malformed X-PAYMENT header."}

        # TRUST GATE — separate, hard, before any money moves. Gates the PAYER's
        ...
        # SETTLEMENT — hidden behind the engine (facilitator/router/token/atomic).
